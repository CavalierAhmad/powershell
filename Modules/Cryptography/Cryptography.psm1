...
function encrypt ($plaintext, $key) {
    ...
    if (-not $key){
        $key = $paths.AESkey
        Write-Host "No key specified, using default key at `$paths.AESkey" -ForegroundColor Yellow
    }
...
function decrypt ($ciphertext, $key) {
    if (-not $ciphertext){Write-Host "Must pass ciphertext to decrypt!" -ForegroundColor Red ; return "${1}error${0}"}
    #if ($ciphertext -is [byte[]]){Write-Host "Forgot to pass ciphertext, dumbass" -ForegroundColor Red ; return "${1}error${0}"}
    if (-not $key){
        $key = $paths.AESkey
        Write-Host "No key specified, using default key at `$paths.AESkey" -ForegroundColor DarkGray
    }
    try {
